...
if len(sys.argv) > 1:
    ...
    print """
    \tUsage:\n\t\tbedFromMaskedFasta.py <fasta file | .fa or .fa.gz> <chrome find> <chrome replace>
    ...
    \t\t\tpython bedFromMaskedFasta.py my_fasta.fa CHROMSOME_ chr
    \t\tOutput is to stdout
    """
    raise SystemExit
...
with input_fasta as f:
    for line in f:
        ...
        if line.startswith(">"):
            ...
            for char in line:
                if state == 0 and char != "3":
                    state = 1
                    start = n
                elif state == 1 and char == "3":
                    state = 0
                    end = n
